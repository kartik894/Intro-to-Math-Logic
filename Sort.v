...
Proof.
  ...
  - simpl. destruct (x <=? a).
    ...
    + simpl. rewrite perm_swap. apply perm_skip. apply IHl.
Qed.
(** [] *)
...
Proof.
  intros. induction l.
  - simpl. apply Permutation_refl.
  - simpl. apply (perm_skip a) in IHl.
    eapply Permutation_trans. apply IHl. apply insert_perm.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. bdestruct (a <=? x).
    ...
    + apply sorted_cons. omega. apply sorted_1.
  - simpl. bdestruct (a <=? x).
    + apply sorted_cons. apply H1. apply sorted_cons. apply H. apply H0.
    + bdestruct (a <=? y).
      {apply sorted_cons. omega. apply sorted_cons. omega. apply H0. }
      {apply sorted_cons. apply H. simpl in IHsorted. apply Nat.leb_gt in H2. rewrite H2 in IHsorted. apply IHsorted. }
Qed.
(** [] *)
...
(** $Date: 2017-10-14 11:47:41 -0500 (Sat, 14 Oct 2017) $ *)
