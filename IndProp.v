...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros n H'. inversion H'. inversion H0. apply H2.
Qed.
(** [] *)
...
Theorem even5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof.
  intros H'. inversion H'. inversion H0. inversion H2.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl in h1. inversion h1. apply IHh2 in H0. apply H0.
Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
  | e : forall l, subseq [] l
  | c1 : forall x l1 l2, subseq l1 l2 -> subseq l1 (x::l2)
  | c2 : forall x l1 l2, subseq l1 l2 -> subseq (x::l1) (x::l2).
...
Proof.
  ...
  - simpl in H. destruct (beq_natP n x).
    + inversion H.
    + apply IHl in H. intros h. inversion h.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | n0: nostutter []
  | n1: forall (x:X), nostutter (x :: [])
  | n2: forall (x y: X) (l: list X),
...
Inductive helper_1 {X} : list X -> list X -> list X -> Prop :=
| h1 : forall l, helper_1 l l []
| h2 : forall l, helper_1 l [] l
| h3 : forall l l1 l2 x, helper_1 l l1 l2 -> helper_1 (x::l) (x::l1) l2
| h4 : forall l l1 l2 x, helper_1 l l1 l2 -> helper_1 (x::l) l1 (x::l2).

Lemma interleaving : forall {X} l l1 l2 (x:X),
    helper_1 (x::l) l1 l2 -> exists l1' l2', l1 = (x::l1') \/ l2 = (x::l2').
Proof.
  intros. inversion H.
  - exists l, []. left.
    reflexivity.
...
Lemma filter_id : forall {X} l test,
    (forall (x:X), In x l -> test x = true) -> filter test l = l.
Proof.
  intros. induction l.
  - reflexivity.
  - assert (test x = true). { apply H. repeat constructor. }
    simpl. rewrite H0. apply f_equal.
    apply IHl. intros.
    apply H. simpl. right. apply H1.
Qed.

Lemma filter_empty : forall {X} l test,
    (forall x:X, In x l -> test x = false) -> filter test l = [].
Proof.
  intros.
  induction l.
  - reflexivity.
  - simpl. assert (test x = false).
    { apply H. simpl. left. reflexivity. }
...
Theorem interleave_filter : forall {X} l l1 l2 (test:X->bool)
                         (Hl1 : forall x, In x l1 -> test x = true)
                         (Hl2 : forall y, In y l2 -> test y = false),
    helper_1 l l1 l2 -> filter test l = l1.
Proof.
  intros.
  induction H.
  - destruct l.
    + reflexivity.
    + apply filter_id. apply Hl1.
  - destruct l.
    + reflexivity.
    + apply filter_empty.
      apply Hl2.
  - simpl. assert (test x = true). { apply Hl1. simpl. left. reflexivity. }
    rewrite H0. apply f_equal. apply IHhelper_1.
    + intros. apply Hl1. simpl. right. apply H1.
    + intros. apply Hl2. apply H1.
  - assert (test x = false). { apply Hl2. simpl. left. reflexivity. }
    simpl. rewrite H0. apply IHhelper_1.
    + apply Hl1.
    + intros. apply Hl2. simpl. right. apply H1.
Qed.
(** [] *)
