...
Induction provides induction hypothesis but destruct doesn't. 
*)
(** [] *)
...
(** Translate your solution for [plus_comm] into an informal proof:
    ...
    Theorem: Addition is commutative. For any [n] and [m], n + m = m + n.
...
Proof.
  ...
  - simpl. rewrite <- mult_n_Sm. rewrite -> IHn. rewrite -> plus_comm. reflexivity.
Qed.
(** [] *)
...
match n with
| BO => O
| D n' => mult_2 (bin_to_nat n')
| MA n' => S (mult_2 (bin_to_nat n'))
end.

Theorem bin_to_nat_pres_incr : forall b:bin,
  S (bin_to_nat b) = bin_to_nat (incr_bin b).
Proof.
  intros. induction b.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
(**
(b) Zero has many representations - BO, D BO, D (D BO), MA (D BO), D (MA BO). There is
no unique representation for binary zero.
**)

Fixpoint helper_norm1 (b : bin) : bin :=
  match b with
    | BO => BO
    | D b' => D (D b')
    | MA b'' => D (MA b'')
...
Fixpoint normalize (b : bin) : bin :=
  match b with
    | BO => BO
    | D b' => helper_norm1 (normalize b')
    | MA b'' => MA (normalize b'')
  end.

Theorem helper_norm3 : forall b : bin,
  helper_norm1 (incr_bin b) = incr_bin (incr_bin (helper_norm1 b)).
Proof.
  intros. destruct b.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Theorem helper_norm2 : forall n : nat,
  nat_to_bin (mult_2 n) = helper_norm1 (nat_to_bin n).
Proof.
  intros. induction n.
  - simpl. reflexivity.
  - simpl. rewrite -> helper_norm3. rewrite <- IHn. reflexivity.
Qed. 

Theorem helper_norm4 : forall b : bin,
  incr_bin (helper_norm1 b) = MA b.
Proof.
  intros. destruct b.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Proof.
  intros. induction b.
  - simpl. reflexivity.
  - simpl. rewrite <- IHb. rewrite -> helper_norm2. reflexivity.
  - simpl. rewrite <- IHb. rewrite -> helper_norm2. rewrite -> helper_norm4. reflexivity. 
Qed. 
